...
# Recomendo que esse código seja rodado em "glowscript.org".
...
xs = slider(bind=xShift,min=-60,max=60,value=0)
...
zs = slider(bind=zShift,min=-45,max=45,value=0)
...
while t < tfinal:
    ...
    if INICIAR:
        ...
        if 60.1 <= bola.pos.x <= 64.6 and 4.7 <= bola.pos.y <= 5.1 and -7.22 <= bola.pos.z <= 7.42:
            bolap.y = (-1)*bolap.y     
            t = t + tfinal/3     # Apenas para acabar mais rápido  
